...
|----|-----------------------------------------------|-------------|-----------------------------------------------------------------------------------------------------------|
| A  | Namespace prefix (`marketnow_*`)              | ✅ Validated | All 15 tools use `marketnow_` prefix + snake_case. No `camelCase`, no mixed-version suffixes.            |
...
| 14 | *(restored v1.15.0)*   | `marketnow_check_revocation`     |
| 15 | *(restored v1.15.0)*   | `marketnow_fingerprint_tool`     |
...
- **v1.15.0** (2026-09-26): **INTEGRATIONS — first-class Docker, Cline and Cursor support + metadata repair.** New production container `mcp-server/Dockerfile` (multi-stage, `npm ci --omit=dev --ignore-scripts`, unprivileged `node` user, read-only-rootfs-friendly, OCI labels) published by CI to `ghcr.io/alicelabs-llc/marketnow-mcp` (pinned + `latest` tags) on every push to master; `docker-compose.yml` mirrors the least-privilege runtime (`--init --read-only --cap-drop ALL --security-opt no-new-privileges`, user 1000:1000). New CI handshake gate `scripts/docker-smoke-test.sh`: a real MCP session (`initialize` + `tools/list`) is piped into the container — asserts `serverInfo.name`, `serverInfo.version == package.json` (image-level drift guard), the exact 15-tool `marketnow_*` surface, and that stdout carries only JSON-RPC frames; a failing image is never pushed. **TOOL-SURFACE RESTORATION (same release):** the 1.15.0 source tree was missing `marketnow_check_revocation` (OCSP-style revocation status, MNR-CRL-1.0) and `marketnow_fingerprint_tool` (TFP-1.0 tool fingerprinting + drift detection) — both live in the published npm 1.14.x series; publishing 1.15.0 without them would have REGRESSED the package surface from 15 to 13 tools. Both tools restored verbatim from the published 1.14.1 tarball (fail-closed responder semantics; self-contained JCS + sha256 fingerprinting), the Docker catalog entry and smoke-test surface updated to 15. Cline/Cursor defaults now ship the npm-npx channel as the PRIMARY entry (works for every user, no Docker login) with the repo-exact Docker image as the documented alternative while the GHCR package is private. New `docker/mcp-registry-entry.yaml` — ready-to-submit entry for the Docker MCP Registry/Toolkit (one-command `docker mcp gateway add` install, least-privilege runtime posture, explicit `marketnow.site/api/*`-only egress declaration). New Cursor integration: `.cursor/mcp.json` (project-level server config Cursor auto-loads) + `.cursor/rules/marketnow.mdc` (house rules bound to `mcp-server/**`, `docker/**`, `integrations/**`, `scripts/**`, workflows). New Cline integration: `integrations/cline/cline_mcp_settings.json` (paste-ready block, Docker primary + pinned-npx alternative, read-only tools pre-whitelisted in `autoApprove`) + `.clinerules/00-marketnow.md` (Cline auto-loads workspace rules). METADATA REPAIR (drift found by the 2026-09-26 external audit round): `server.json` pointed at the WRONG repository (`edgarfloresguerra2011-a11y/marketnow` → `github.com/alicelabs-llc/MARKETNOW`) and the WRONG license (`"Proprietary"` → `MIT`, matching package.json + LICENSE); `package.json mcpName` corrected to `io.github.alicelabs-llc/marketnow`; stale skills count (8,764 → 9,248, from public/stats.json). VERSION-DRIFT FIX: `index.js` hand-coded THREE different versions (header comment v1.10.0, `serverInfo.version` v1.10.1, banner v1.10.1) while package.json said 1.11.0 — the server now reads `PKG_VERSION` from `package.json` at startup, so the MCP handshake, the banner and the npm manifest are structurally unable to drift. Version chosen as 1.15.0 (above the npm 1.12–1.14.1 series published pre-history-rewrite, so repo and npm version trains stay unambiguous). `scripts/check-version-sync.mjs` extended: server.json repository/name/license vs package.json, `.cursor/mcp.json` + Cline settings JSON validity and version pin, Dockerfile ARG VERSION, docker-compose/compose-catalog/Docker-registry-entry version mentions, and a regression guard asserting index.js contains no hardcoded semver. New workflow `.github/workflows/integrations-ci.yml`: docker job (build → smoke handshake → push GHCR), configs job (JSON/YAML validation + integration version checks + version-sync).
- **v1.11.0** (2026-09-26): **SECURITY — fail-closed trust verification.** `marketnow_verify_atc_spec` now defaults to TRUST mode: a caller-supplied trusted CA (`ca_public_key`, obtained out-of-band) is REQUIRED — without it the result is `DENY`/`CONFIGURATION_ERROR`, because a malicious ATC can carry the very key that verifies its own signature (the v1.10.x fallback to `atc.issuer.ca_public_key` on the trust path was removed). New fail-closed revocation enforcement: when the card sets `revocation_check_required=true`, the caller must supply `revocation_status` (pre-fetched CRL list or `{revoked:boolean}` status) — a valid signature never means currently-trusted; no evidence → DENY. New `mode:'self_described'` (explicit opt-in, debugging/interop only) verifies the signature math against the card-embedded key and is labeled `trust_decision:'NOT_APPLICABLE'` — impossible to confuse with a trust decision. Results now carry `verification_mode` and `trust_decision`. Lib exports add `verifyTrust()` and `verifySelfDescribedSignature()`. Same fix shipped in `agent-trust-card@1.2.0` (SDK: `verifyTrust()` async wrapper with CRL auto-fetch, sync fail-closed via `options.revocation`, self-described mode loudly labeled). Version sync: package.json and server.json aligned at 1.11.0; CI now checks version consistency across package.json / server.json / README / AUDIT.md.
